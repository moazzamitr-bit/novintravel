...
function Field({
  ...
  return (
    <label className="block text-[13px] text-novin-text-secondary">
      <span className="mb-1.5 block font-medium text-novin-text">{label}</span>
      {children}
    </label>
  );
}
...
const inputClass =
  "h-11 w-full rounded-xl border border-novin-border bg-white px-3 text-[14px] text-novin-text outline-none transition focus:border-novin-purple focus:ring-2 focus:ring-novin-purple/20";
  ...
  return (
    <div className="container-page section-spacing !pt-8">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[13px] text-novin-orange">نسخه دمو فلوی رزرو</p>
          <h1 className="mt-1 text-[24px] font-bold text-novin-text sm:text-[28px]">
            اطلاعات مسافران
          </h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="space-y-5">
          {draft.passengers.map((passenger, index) => (
            <section
              key={index}
              className="rounded-[22px] border border-novin-border bg-white p-5 sm:p-6"
            >
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                {draft.passengers.length > 1 ? (
                  <button
                    ...
                    className="text-[13px] text-novin-orange hover:underline"
                  >
                    حذف
                  </button>
                ) : null}
          ...
          <button
            ...
            className="w-full rounded-[18px] border border-dashed border-novin-purple/40 bg-[color-mix(in_srgb,var(--color-novin-purple)_4%,white)] px-4 py-3 text-[14px] font-medium text-novin-purple hover:bg-[color-mix(in_srgb,var(--color-novin-purple)_8%,white)]"
          >
            + افزودن مسافر
          </button>

          <section className="rounded-[22px] border border-novin-border bg-white p-5 sm:p-6">
            <h2 className="text-[16px] font-bold text-novin-text">اطلاعات تماس</h2>
          ...
          <section className="rounded-[22px] border border-novin-border bg-white p-5 sm:p-6">
            <h2 className="text-[16px] font-bold text-novin-text">خدمات تکمیلی</h2>
            <div className="mt-4 space-y-3">
              <label className="flex cursor-pointer items-start gap-3 rounded-2xl bg-novin-bg-secondary p-4">
                <input
                  type="checkbox"
                  className="mt-1 h-4 w-4 accent-[var(--color-novin-purple)]"
                ...
                />
                <span>
                  <span className="block text-[14px] font-semibold text-novin-text">
                    بسته استرداد منعطف
                  </span>
                  <span className="mt-1 block text-[13px] text-novin-text-secondary">
          ...
          {error ? (
            <p className="rounded-2xl bg-orange-50 px-4 py-3 text-[13px] text-novin-orange">
              {error}
            </p>
          ) : null}
