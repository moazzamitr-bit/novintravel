    ...
    <nav aria-label="مسیر صفحه" className="mb-6">
      <ol className="flex flex-wrap items-center gap-1 text-[13px] text-novin-text-muted">
        ...
        <li className="inline-flex items-center gap-1">
          ...
          <Link href={parentHref} className="hover:text-novin-purple">
            {parentLabel}
          </Link>
        </li>
        <li className="inline-flex items-center gap-1">
          <ChevronLeft className="h-3.5 w-3.5" aria-hidden />
          <span className="text-novin-text-secondary">{current}</span>
        </li>
      </ol>
    </nav>
  );
...
function StickyBookCard({
  ...
  return (
    <aside className="h-fit rounded-[22px] border border-novin-border bg-white p-5 shadow-card lg:sticky lg:top-28">
      <p className="text-[12px] text-novin-text-muted">شروع قیمت</p>
      <p className="mt-1 text-[26px] font-bold text-novin-purple">
        {priceLabel(price)}
      </p>
        ...
        <div>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 inline-flex items-center gap-1.5 text-[14px] text-novin-text-secondary">
                <MapPin className="h-4 w-4 text-novin-purple" />
                {hotel.address}
              </p>
            </div>
            <div className="inline-flex items-center gap-2 rounded-2xl bg-novin-purple px-3 py-2 text-white">
          ...
          <p className="mt-5 max-w-3xl text-[15px] leading-8 text-novin-text-secondary">
            {hotel.description}
          </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {hotel.tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-novin-bg-secondary px-3 py-1.5 text-[12px] text-novin-text-secondary"
              >
                {tag}
              </span>
            ))}
          ...
          <section className="mt-8">
            <h2 className="text-[18px] font-bold text-novin-text">امکانات هتل</h2>
            <ul className="mt-4 grid gap-3 sm:grid-cols-2">
              {hotel.amenities.map((item) => (
                <li
                  key={item}
                  className="inline-flex items-center gap-2 rounded-2xl border border-novin-border bg-white px-4 py-3 text-[14px] text-novin-text"
                >
                  <Wifi className="h-4 w-4 text-novin-purple" />
                  {item}
                </li>
              ))}
            </ul>
          ...
          <section className="mt-8">
            ...
            <div className="mt-4 space-y-3">
              {hotel.rooms.map((room) => (
                <article
                  key={room.id}
                  className="flex flex-col gap-4 rounded-[18px] border border-novin-border bg-white p-4 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div>
    ...
    <div className="container-page section-spacing !pt-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div>
          ...
          <p className="mt-2 text-[15px] text-novin-text-secondary">
          ...
          <section className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="rounded-[18px] border border-novin-border bg-white p-5">
              <h2 className="text-[16px] font-bold text-novin-text">شامل می‌شود</h2>
            ...
            <div className="rounded-[18px] border border-novin-border bg-white p-5">
              <h2 className="text-[16px] font-bold text-novin-text">شامل نمی‌شود</h2>
              <ul className="mt-3 space-y-2 text-[14px] text-novin-text-secondary">
          ...
          <section className="mt-8">
            <h2 className="text-[18px] font-bold text-novin-text">برنامه سفر</h2>
            <ol className="mt-4 space-y-3">
              {tour.itinerary.map((day) => (
                <li
                  key={day.day}
                  className="rounded-[18px] border border-novin-border bg-white p-4"
                >
          ...
          <section className="mt-8">
            <h2 className="text-[18px] font-bold text-novin-text">تاریخ‌های نزدیک</h2>
            <div className="mt-3 flex flex-wrap gap-2">
              {tour.nextDates.map((date) => (
                <span
                  key={date}
                  className="rounded-full bg-novin-bg-secondary px-3 py-1.5 text-[13px] text-novin-text"
                >
                  {date}
                </span>
              ))}
    ...
    <div className="container-page section-spacing !pt-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-[13px] text-novin-text-secondary">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-novin-bg-secondary px-3 py-1.5">
              <Users className="h-3.5 w-3.5" />
              {stay.guests.toLocaleString("fa-IR")} مهمان
            </span>
            <span className="rounded-full bg-novin-bg-secondary px-3 py-1.5">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-novin-purple px-3 py-1.5 text-white">
          ...
          <p className="mt-3 text-[14px] font-medium text-novin-text">{stay.host}</p>

          <section className="mt-8">
            <h2 className="text-[18px] font-bold text-novin-text">امکانات</h2>
            <ul className="mt-4 grid gap-3 sm:grid-cols-2">
              {stay.amenities.map((item) => (
                <li
                  key={item}
                  className="rounded-2xl border border-novin-border bg-white px-4 py-3 text-[14px] text-novin-text"
                >
                  {item}
                </li>
              ))}
