...
const faqs = [
  ...
  },
  {
    ...
    a: "از طریق شماره ۰۲۱-۴۱۵۶۷، ایمیل support@novintravel.ir یا فرم تیکت در صفحه پشتیبانی.",
  },
];
    ...
    <div className="mx-auto max-w-3xl space-y-3">
      {faqs.map((item, index) => {
        ...
        return (
          <div
            key={item.q}
            className="overflow-hidden rounded-[18px] border border-novin-border bg-white"
          >
            <button
              type="button"
            ...
            >
              <span className="text-[15px] font-bold text-novin-text">
                {item.q}
              </span>
              <ChevronDown
                className={cn(
                  "h-5 w-5 shrink-0 text-novin-purple transition-transform",
                  isOpen && "rotate-180",
                )}
              />
            </button>
            {isOpen ? (
              <p className="border-t border-novin-border px-5 py-4 text-[14px] leading-7 text-novin-text-secondary">
                {item.a}
              </p>
            ) : null}
          </div>
