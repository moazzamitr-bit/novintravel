...
function DemoBadge() {
  return (
    <span className="inline-flex rounded-full bg-novin-orange/10 px-3 py-1 text-[12px] font-medium text-novin-orange">
      نسخه دمو · داده نمونه
    </span>
  );
}
    ...
    <>
      ...
      <section className="container-page section-spacing !pt-8">
        <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
          ...
          <div className="rounded-[24px] border border-novin-border bg-white p-5 sm:p-6">
            <h2 className="text-[18px] font-bold text-novin-text">تراکنش‌های اخیر</h2>
            <ul className="mt-4 space-y-3">
              {tx.map((item) => (
                <li
                  key={item.id}
                  className="flex items-center justify-between gap-3 rounded-2xl bg-novin-bg-secondary px-4 py-3"
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`inline-flex h-9 w-9 items-center justify-center rounded-xl ${
                        item.type === "in"
                          ...
                          : "bg-orange-50 text-novin-orange"
                      }`}
                    >
                      {item.type === "in" ? (
                    ...
                    <div>
                      <p className="text-[14px] font-semibold text-novin-text">
                        {item.title}
                      </p>
                      <p className="text-[12px] text-novin-text-muted">{item.date}</p>
                    </div>
                  </div>
                  <p
                    className={`text-[14px] font-bold ${
                      item.type === "in" ? "text-emerald-600" : "text-novin-text"
                    }`}
                  >
                    {item.type === "in" ? "+" : "-"}
...
export function ClubDemoPage() {
  ...
  return (
    <>
      ...
      <section className="container-page section-spacing !pt-8">
        <div className="rounded-[24px] border border-novin-border bg-white p-6 sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-[13px] text-novin-orange">سطح فعلی: نقره‌ای</p>
              <h2 className="mt-1 text-[24px] font-bold text-novin-text">
                ۲٬۴۸۰ امتیاز
              </h2>
              <p className="mt-2 text-[14px] text-novin-text-secondary">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {tiers.map((tier) => (
            <div
              key={tier.name}
              className="rounded-[20px] border border-novin-border bg-white p-5"
            >
              <Gift className="h-5 w-5 text-novin-purple" />
              <h3 className="mt-3 text-[17px] font-bold text-novin-text">
                {tier.name}
              </h3>
              <p className="mt-1 text-[13px] text-novin-text-muted">{tier.points}</p>
              <p className="mt-3 text-[14px] text-novin-text-secondary">{tier.perk}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="container-page section-spacing !pt-8">
        <div className="grid gap-3 sm:grid-cols-4">
          {steps.map((step, index) => (
            <div
              key={step}
              className="rounded-2xl border border-novin-border bg-white p-4 text-center"
            >
              <p className="text-[12px] text-novin-orange">
                مرحله {(index + 1).toLocaleString("fa-IR")}
              </p>
              <p className="mt-1 text-[14px] font-bold text-novin-text">{step}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 space-y-4">
          {tickets.map((ticket) => (
            <article
              key={ticket.id}
              className="flex flex-col gap-4 rounded-[20px] border border-novin-border bg-white p-5 sm:flex-row sm:items-center sm:justify-between"
            >
              <div>
                <p className="text-[12px] text-novin-text-muted" dir="ltr">
                  {ticket.id}
                </p>
                <h3 className="mt-1 text-[16px] font-bold text-novin-text">
                  {ticket.product}
                </h3>
          ...
          {plans.map((plan) => (
            <article
              ...
              className={`rounded-[22px] border p-6 ${
                plan.featured
                  ? "border-novin-purple bg-[color-mix(in_srgb,var(--color-novin-purple)_6%,white)] shadow-card"
                  : "border-novin-border bg-white"
              }`}
            >
              <div className="flex items-center gap-2">
                <ShieldCheck className="h-5 w-5 text-novin-purple" />
                <h2 className="text-[18px] font-bold text-novin-text">{plan.name}</h2>
              </div>
              <p className="mt-4 text-[28px] font-bold text-novin-purple">
                {formatToman(plan.price)}
              </p>
              <p className="text-[13px] text-novin-text-muted">پوشش {plan.cover}</p>
              <ul className="mt-5 space-y-2 text-[14px] text-novin-text-secondary">
                {plan.items.map((item) => (
                  <li key={item}>• {item}</li>
                ))}
              </ul>
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="group rounded-[20px] border border-novin-border bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-card"
              >
                <span className="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-[color-mix(in_srgb,var(--color-novin-purple)_10%,white)] text-novin-purple">
                  <Icon className="h-5 w-5" />
                </span>
                <h2 className="mt-4 text-[17px] font-bold text-novin-text group-hover:text-novin-purple">
                  {item.title}
                </h2>
                <p className="mt-1 text-[14px] text-novin-text-secondary">{item.desc}</p>
              </Link>
            );
          })}
        </div>
  ...
  return (
    <>
      ...
      <section className="container-page section-spacing !pt-8">
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            <div
              ...
              className="rounded-[20px] border border-novin-border bg-white p-5"
            >
              <p className="text-[13px] text-novin-text-muted">{stat.label}</p>
              <p className="mt-2 text-[28px] font-bold text-novin-purple">
                {stat.value}
              </p>
            </div>
          ))}
        ...
        <div className="mt-6 overflow-hidden rounded-[22px] border border-novin-border bg-white">
          <div className="border-b border-novin-border px-5 py-4">
            <h2 className="text-[16px] font-bold text-novin-text">
              درخواست‌های اخیر (دمو)
            </h2>
          </div>
          <div className="divide-y divide-novin-border">
            {[
              ...
              >
                <span className="font-medium text-novin-text">{team}</span>
                <span className="text-novin-text-secondary">{route}</span>
                <span className="text-novin-purple sm:text-left">{status}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="container-page section-spacing !pt-8">
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            <div
              key={item.label}
              className="rounded-[20px] bg-[linear-gradient(160deg,#f7f4fb,#fff)] border border-novin-border p-6 text-center"
            >
              <p className="text-[28px] font-bold text-novin-purple">{item.value}</p>
              <p className="mt-1 text-[13px] text-novin-text-secondary">{item.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          {[
            ...
            <article
              ...
              className="rounded-[20px] border border-novin-border bg-white p-5"
            >
              <Building2 className="h-5 w-5 text-novin-purple" />
              <h2 className="mt-3 text-[17px] font-bold text-novin-text">
                {item.title}
              </h2>
...
export function PartnersDemoPage() {
  return (
    <>
      <PageHero
        title="همکاری با ما"
        description="آژانس، تأمین‌کننده یا شریک فناوری؛ با نوین تراول رشد کنید."
      ...
      <section className="container-page section-spacing !pt-8">
          ...
          {[
            ...
            return (
              <article
                ...
                className="rounded-[20px] border border-novin-border bg-white p-5"
              >
                <Icon className="h-5 w-5 text-novin-purple" />
                <h2 className="mt-3 text-[17px] font-bold text-novin-text">
                  {item.title}
                </h2>
                <p className="mt-2 text-[14px] leading-7 text-novin-text-secondary">
        ...
        <div className="mt-8 rounded-[22px] border border-novin-border bg-novin-bg-secondary p-6 sm:p-8">
          <h2 className="text-[20px] font-bold text-novin-text">فرم درخواست همکاری (دمو)</h2>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <input
              className="h-12 rounded-xl border border-novin-border bg-white px-4 text-[14px]"
            ...
            />
            <select className="h-12 rounded-xl border border-novin-border bg-white px-4 text-[14px] sm:col-span-2">
          ...
          {jobs.map((job) => (
            ...
            >
              <div className="flex items-start gap-3">
                <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-[color-mix(in_srgb,var(--color-novin-purple)_10%,white)] text-novin-purple">
      ...
      <section className="container-page section-spacing !pt-8">
        <div className="grid gap-4 md:grid-cols-3">
          {[
            ...
            { icon: FileText, title: "ایمیل", value: "support@novintravel.ir" },
            ...
            return (
              ...
              >
                <Icon className="h-5 w-5 text-novin-purple" />
                <p className="mt-3 text-[13px] text-novin-text-muted">{item.title}</p>
                <p className="mt-1 text-[16px] font-bold text-novin-text">{item.value}</p>
              </div>
            );
          })}
        </div>

        <div className="mt-8 rounded-[22px] border border-novin-border bg-white p-6 sm:p-8">
          <h2 className="text-[18px] font-bold text-novin-text">ثبت تیکت (دمو)</h2>
          <div className="mt-4 grid gap-3">
            <input
              className="h-12 rounded-xl border border-novin-border px-4 text-[14px]"
              placeholder="موضوع"
            />
            <textarea
