  ...
  return (
    <section className="container-page section-spacing !pt-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[13px] text-novin-text-muted">نمایش دمو · داده نمونه</p>
          <h2 className="mt-1 text-[22px] font-bold text-novin-text sm:text-[26px]">
            {title}
          </h2>
          <p className="mt-1 text-[14px] text-novin-text-secondary">
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="h-fit rounded-[20px] border border-novin-border bg-white p-4 lg:sticky lg:top-28">
          <p className="mb-4 text-[15px] font-bold text-novin-text">فیلترها</p>
          <div className="space-y-5">
            {filterGroups.map((group) => (
              <div key={group.id}>
                <p className="mb-2 text-[13px] font-semibold text-novin-text">
                  {group.title}
                </p>
                <ul className="space-y-2">
                  {group.options.map((option) => {
                    ...
                    return (
                      <li key={option.id}>
                        <label className="flex cursor-pointer items-center justify-between gap-2 text-[13px] text-novin-text-secondary">
                          ...
                          {typeof option.count === "number" ? (
                            <span className="text-novin-text-muted">
