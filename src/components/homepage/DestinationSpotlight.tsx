    ...
    >
      ...
      <div className="container-page section-spacing">
        <div className="mb-8 flex flex-col gap-3 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="mb-2 text-[13px] font-semibold tracking-wide text-novin-orange">
              الهام سفر
            </p>
            <h2 className="text-[26px] font-extrabold text-novin-text sm:text-[32px]">
              ایران را از زاویه‌ای تازه ببینید
            </h2>
            <p className="mt-2 max-w-xl text-[15px] text-novin-text-secondary">
              از حرم تا حافظیه و میدان نقش‌جهان — مقصد بعدی را با حال‌وهوای واقعی انتخاب کنید.
            </p>
          </div>
          <Link
            href="/tours"
            className="inline-flex items-center gap-2 text-[14px] font-semibold text-novin-purple transition hover:text-novin-orange"
          >
            مشاهده تورها
        ...
        <div className="grid gap-4 lg:grid-cols-3 lg:grid-rows-2 lg:gap-5">
          {spots.map((spot) => (
            <Link
              key={spot.id}
              href={spot.href}
              className={`group relative overflow-hidden rounded-[22px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-novin-purple/40 ${spot.className}`}
            >
              <Image
                src={spot.image}
                alt={spot.title}
