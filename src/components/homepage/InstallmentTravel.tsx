...
export function InstallmentTravel() {
  return (
    <section className="container-page section-spacing !pt-0" aria-labelledby="installment-title">
      <div className="grid items-center gap-8 overflow-hidden rounded-[24px] border border-novin-border bg-white p-6 sm:p-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-12 lg:p-10">
        <div>
          <p className="mb-2 text-[13px] font-medium text-novin-orange">
            تمایز استراتژیک نوین
          </p>
          <h2
          ...
          <ul className="mt-6 grid gap-3 sm:grid-cols-3">
            {installmentHighlights.map((item) => (
              ...
              >
                <p className="text-[15px] font-bold text-novin-purple">
                  {item.title}
                </p>
                <p className="mt-1 text-[12.5px] text-novin-text-secondary">
                  {item.description}
                </p>
              </li>
            ))}
