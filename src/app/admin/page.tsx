    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        <PanelCard
          ...
          action={
            <Link href="/admin/reports" className="text-[13px] text-novin-purple">
              گزارش کامل
            </Link>
          }
        >
            ...
            {adminSalesSeries.map((item) => (
              <div key={item.day} className="flex flex-1 flex-col items-center gap-2">
                <div
                  ...
                  title={`${item.value}`}
                />
                <span className="text-[12px] text-novin-text-muted">{item.day}</span>
              </div>
            ))}
          </div>
        </PanelCard>
        ...
        <PanelCard title="تیکت‌های فوری">
          <ul className="space-y-3">
            {adminSupport.slice(0, 3).map((ticket) => (
              <li
                key={ticket.id}
                className="rounded-2xl bg-novin-bg-secondary px-4 py-3"
              >
                <p className="text-[12px] text-novin-text-muted" dir="ltr">
                  {ticket.id}
                </p>
                <p className="mt-1 text-[14px] font-semibold text-novin-text">
                  {ticket.subject}
                </p>
