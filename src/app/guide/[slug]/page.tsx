...
export default async function GuideArticlePage({
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-wrap gap-3 text-[13px] text-novin-text-secondary">
          <span className="rounded-full bg-white px-3 py-1 border border-novin-border">
            {article.category}
          </span>
          <span className="rounded-full bg-white px-3 py-1 border border-novin-border">
            {article.readingTime}
        ...
        <div className="mx-auto max-w-3xl space-y-8">
          {article.sections.map((section) => (
            <section key={section.heading}>
              <h2 className="text-[20px] font-bold text-novin-text">
                {section.heading}
              </h2>
