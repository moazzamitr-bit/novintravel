  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[20px] bg-[linear-gradient(135deg,#4f2f7c_0%,#6b4a96_55%,#f84209_140%)] p-6 text-white sm:p-7">
        ...
        <h2 className="mt-1 text-[24px] font-bold">به پنل نوین تراول خوش آمدید</h2>
      ...
      <PanelCard
        ...
        action={
          <Link href="/account/orders" className="text-[13px] font-medium text-novin-purple">
            همه سفارش‌ها
          </Link>
        }
      >
            ...
            <tbody>
              {accountOrders.slice(0, 4).map((order) => (
                <tr key={order.id} className="border-b border-novin-border/70">
                  ...
                  <td className="py-3 text-novin-text-secondary">{order.type}</td>
                  <td className="py-3 text-novin-text">{order.title}</td>
                  <td className="py-3 text-novin-text">
